...
2026-08-07. GitHub can change plans and prices, so confirm current values in
the linked official documentation before making a spending decision.

## Current repository situation

`marcelpetrick/CroLingo` is a private repository owned by a personal account.
Its workflows use standard GitHub-hosted Ubuntu runners. On GitHub Free this
means that workflow time and retained artifacts consume the repository owner's
included allowance.

The included GitHub Free allowances are:
...
Standard hosted runners are free for public repositories. Self-hosted runner
execution does not consume hosted-runner minutes, although the owner remains
responsible for the machine and its security.
...
A complete CroLingo Quality or Development release job currently takes roughly
15 to 20 hosted-runner minutes. At that rate, 2,000 minutes permit about 100 to
130 complete jobs per month, or approximately 50 to 65 Quality-and-Release
pairs. This is only a planning estimate; the billing view is authoritative.

To stay comfortably within the free allowance:
...
- push coherent, already verified commits instead of using CI as an iterative
  debugger;
- dispatch Development releases only for builds that will actually be tested;
